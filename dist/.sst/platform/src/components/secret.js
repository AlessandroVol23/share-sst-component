...
export class SecretMissingError extends VisibleError {
    constructor(secretName) {
        super(`Set a value for ${secretName} with \`sst secret set ${secretName} <value>\``);
...
export class Secret extends Component {
    ...
    constructor(name, placeholder) {
        super("sst:sst:Secret", name, {
            placeholder,
        }, {});
        this._name = name;
        this._placeholder = placeholder ? output(placeholder) : undefined;
        this._value = output(process.env["SST_SECRET_" + this._name] ?? this._placeholder).apply((value) => {
