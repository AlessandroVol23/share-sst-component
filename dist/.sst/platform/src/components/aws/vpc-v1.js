...
export class Vpc extends Component {
    constructor(name, args, opts) {
        ...
        function createNatGateways() {
            const ret = publicSubnets.apply((subnets) => subnets.map((subnet, i) => {
                const elasticIp = new ec2.Eip(...transform(args?.transform?.elasticIp, `${name}ElasticIp${i + 1}`, {
                    vpc: true,
                }, { parent }));
                const natGateway = new ec2.NatGateway(...transform(args?.transform?.natGateway, `${name}NatGateway${i + 1}`, {
                    subnetId: subnet.id,
    ...
    static get(name, vpcID) {
        ...
        const privateSubnets = ec2
            ...
            .ids.apply((ids) => ids.map((id, i) => ec2.Subnet.get(`${name}PrivateSubnet${i + 1}`, id)));
        const privateRouteTables = privateSubnets.apply((subnets) => subnets.map((subnet, i) => ec2.RouteTable.get(`${name}PrivateRouteTable${i + 1}`, ec2.getRouteTableOutput({ subnetId: subnet.id }).routeTableId)));
        const publicSubnets = ec2
            ...
            .ids.apply((ids) => ids.map((id, i) => ec2.Subnet.get(`${name}PublicSubnet${i + 1}`, id)));
        const publicRouteTables = publicSubnets.apply((subnets) => subnets.map((subnet, i) => ec2.RouteTable.get(`${name}PublicRouteTable${i + 1}`, ec2.getRouteTableOutput({ subnetId: subnet.id }).routeTableId)));
        const natGateways = publicSubnets.apply((subnets) => subnets.map((subnet, i) => ec2.NatGateway.get(`${name}NatGateway${i + 1}`, ec2.getNatGatewayOutput({ subnetId: subnet.id }).id)));
        const elasticIps = natGateways.apply((nats) => nats.map((nat, i) => ec2.Eip.get(`${name}ElasticIp${i + 1}`, nat.allocationId)));
        return new Vpc(name, {
            ref: true,
            vpc,
            internetGateway,
