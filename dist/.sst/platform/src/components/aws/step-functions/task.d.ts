...
export declare class Task extends State implements Nextable, Failable {
    ...
    protected toJSON(): {
        ...
        Timeout: import("@pulumi/pulumi").OutputInstance<number | `${number} second` | `${number} seconds` | `${number} minute` | `${number} minutes` | `${number} hour` | `${number} hours` | `${number} day` | `${number} days` | `{% ${string} %}`> | undefined;
