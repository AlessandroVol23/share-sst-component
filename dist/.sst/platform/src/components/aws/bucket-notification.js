...
export class BucketNotification extends Component {
    constructor(name, args, opts) {
        ...
        function normalizeNotifications() {
            return output(args.notifications).apply((notifications) => notifications.map((n) => {
                const count = (n.function ? 1 : 0) + (n.queue ? 1 : 0) + (n.topic ? 1 : 0);
                if (count === 0)
                    throw new VisibleError(`At least one of function, queue, or topic is required for the "${n.name}" bucket notification.`);
                if (count > 1)
                    throw new VisibleError(`Only one of function, queue, or topic is allowed for the "${n.name}" bucket notification.`);
                return {
                    ...n,
                    events: n.events ?? [
                        "s3:ObjectCreated:*",
        ...
        function createNotificationsConfig() {
            return notifications.apply((notifications) => {
                const config = notifications.map((n) => {
                    if (n.function) {
                        const fn = functionBuilder(`${name}Notification${n.name}`, n.function, {
                        ...
                        const permission = new lambda.Permission(`${name}Notification${n.name}Permission`, {
                    ...
                    if (n.topic) {
                        const arn = n.topic instanceof SnsTopic ? n.topic.arn : output(n.topic);
                        const policy = new sns.TopicPolicy(`${name}Notification${n.name}Policy`, {
                            arn,
                    ...
                    if (n.queue) {
                        const arn = n.queue instanceof Queue ? n.queue.arn : output(n.queue);
                        const policy = Queue.createPolicy(`${name}Notification${n.name}Policy`, arn, { parent: self });
        ...
        function createNotification() {
            return new s3.BucketNotification(...transform(args.transform?.notification, `${name}Notification`, {
                bucket: bucket.name,
                lambdaFunctions: config.apply((config) => config
                    .filter((c) => c.functionBuilder)
                    .map((c) => ({
                    id: c.args.name,
                ...
                queues: config.apply((config) => config
                    .filter((c) => c.queueArn)
                    .map((c) => ({
                    id: c.args.name,
                    queueArn: c.queueArn,
                ...
                topics: config.apply((config) => config
                    .filter((c) => c.topicArn)
                    .map((c) => ({
                    id: c.args.name,
                    topicArn: c.topicArn,
                ...
                parent: self,
                dependsOn: config.apply((config) => config.map((c) => c.dependsOn)),
            }));
        }
    }
    /**
    ...
    get nodes() {
        ...
        return {
             ...
             */
            get functions() {
                return output(self.functionBuilders).apply((functionBuilders) => functionBuilders.map((builder) => builder.getFunction()));
            },
            /**
