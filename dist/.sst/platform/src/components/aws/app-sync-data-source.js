...
export class AppSyncDataSource extends Component {
    constructor(name, args, opts) {
        ...
        function validateSingleDataSource() {
            ...
            if (sources.length > 1) {
                throw new Error(`Expected only one data source, but found ${sources.length}.`);
            }
        }
        function getType() {
            if (args.lambda)
        ...
        function createServiceRole() {
            if (!lambda &&
                ...
                !args.openSearch)
                return;
            return new iam.Role(...transform(args.transform?.serviceRole, `${name}ServiceRole`, {
        ...
        function createDataSource() {
            return new appsync.DataSource(...transform(args.transform?.dataSource, `${name}DataSource`, {
                apiId,
                type,
                name: args.name,
                ...
                dynamodbConfig: args.dynamodb
                    ? {
                        tableName: output(args.dynamodb).apply((arn) => parseDynamoArn(arn).tableName),
                    }
                    : undefined,
    ...
    get nodes() {
        ...
        return {
            ...
            get serviceRole() {
                if (!self.serviceRole)
                    throw new VisibleError("Cannot access `nodes.serviceRole` because the data source does not have a service role.");
