    ...
    constructor(name, args, opts) {
        ...
        function validateSingleAuthorizer() {
            ...
            if (authorizers.length === 0)
                throw new VisibleError(`Please provide one of "lambda" or "jwt" for the ${args.name} authorizer.`);
            if (authorizers.length > 1)
                throw new VisibleError(`Please provide only one of "lambda" or "jwt" for the ${args.name} authorizer.`);
        }
        function createFunction() {
            if (!lamb)
                return;
            return functionBuilder(`${name}Handler`, lamb.function, {
                description: interpolate `${api.name} authorizer`,
            }, undefined, { parent: self });
        }
        function createAuthorizer() {
            const defaultIdentitySource = args.type === "http"
                ? "$request.header.Authorization"
                : "route.request.header.Authorization";
            return new apigatewayv2.Authorizer(...transform(args.transform?.authorizer, `${name}Authorizer`, {
                apiId: api.id,
                ...(lamb
                    ? {
                        authorizerType: "REQUEST",
                        identitySources: lamb.apply((lamb) => lamb.identitySources ?? [defaultIdentitySource]),
                        authorizerUri: fn.invokeArn,
                        ...(args.type === "http"
                            ? {
                                authorizerResultTtlInSeconds: lamb.apply((lamb) => toSeconds(lamb.ttl ?? "0 seconds")),
                                authorizerPayloadFormatVersion: lamb.apply((lamb) => lamb.payload ?? "2.0"),
                                enableSimpleResponses: lamb.apply((lamb) => (lamb.response ?? "simple") === "simple"),
                            }
                            : {}),
                    }
                    : {
                        authorizerType: "JWT",
                        identitySources: [
                            jwt.apply((jwt) => jwt.identitySource ?? defaultIdentitySource),
                        ],
