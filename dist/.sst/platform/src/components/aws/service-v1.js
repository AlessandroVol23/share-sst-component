...
import { all, interpolate, output, secret, } from "@pulumi/pulumi";
...
import { supportedCpus, supportedMemories, } from "./cluster-v1.js";
import { RETENTION } from "./logging.js";
import { URL_UNAVAILABLE } from "./linkable.js";
import { appautoscaling, cloudwatch, ec2, ecr, ecs, getCallerIdentityOutput, getRegionOutput, iam, lb, } from "@pulumi/aws";
...
export class Service extends Component {
    constructor(name, args, opts) {
            ...
            if (args.vpc instanceof Vpc) {
                ...
                return args.vpc.nodes.natGateways.apply((natGateways) => {
                    if (natGateways.length === 0)
                        throw new VisibleError(`The VPC configured for the service does not have NAT enabled. Enable NAT by configuring "nat" on the "sst.aws.Vpc" component.`);
                    return result;
                });
            }
            // "vpc" is object
        ...
        function normalizeStorage() {
            return output(args.storage ?? "21 GB").apply((v) => {
                ...
                if (storage < 21 || storage > 200)
                    throw new Error(`Unsupported storage: ${v}. The supported value for storage is between "21 GB" and "200 GB"`);
                return v;
            });
        }
        ...
        function normalizePublic() {
            ...
            const ports = output(args.public).apply((pub) => {
                ...
                if (!pub.ports || pub.ports.length === 0)
                    throw new VisibleError(`You must provide the ports to expose via "public.ports".`);
                ...
                const appProtocols = ports.filter((port) => ["http", "https"].includes(port.listenProtocol) &&
                ...
                ports.forEach((port) => {
                    if (["https", "tls"].includes(port.listenProtocol) && !pub.domain) {
                        throw new VisibleError(`You must provide a custom domain for ${port.listenProtocol.toUpperCase()} protocol.`);
                    }
                });
                return ports;
            });
            const domain = output(args.public).apply((pub) => {
                if (!pub.domain)
                    return undefined;
                // normalize domain
                const domain = typeof pub.domain === "string" ? { name: pub.domain } : pub.domain;
                return {
                    name: domain.name,
        ...
        function createImage() {
            ...
            const imageArgsNew = imageArgs.apply((imageArgs) => {
                ...
                if (!lines.find((line) => line === ".sst")) {
                    fs.writeFileSync(file ?? path.join(context, ".dockerignore"), [...lines, "", "# sst", ".sst"].join("\n"));
                }
                return imageArgs;
            });
            // Build image
            return new Image(...transform(args.transform?.image, `${name}Image`, {
                context: {
                    location: imageArgsNew.apply((v) => path.join($cli.paths.root, v.context)),
                },
                dockerfile: {
                    location: imageArgsNew.apply((v) => v.dockerfile
        ...
        function createLoadBalancer() {
            ...
            const loadBalancer = new lb.LoadBalancer(...transform(args.transform?.loadBalancer, `${name}LoadBalancer`, {
                internal: false,
                loadBalancerType: pub.ports.apply((ports) => ports[0].listenProtocol.startsWith("http")
                    ? "application"
                    : "network"),
            ...
            const ret = all([pub.ports, certificateArn]).apply(([ports, cert]) => {
                ...
                ports.forEach((port) => {
                    ...
                    const listener = listeners[listenerId] ??
                        new lb.Listener(...transform(args.transform?.listener, `${name}Listener${listenerId}`, {
        ...
        function createTaskRole() {
            const policy = all([args.permissions || [], linkPermissions]).apply(([argsPermissions, linkPermissions]) => iam.getPolicyDocumentOutput({
                statements: [...argsPermissions, ...linkPermissions].map((item) => ({
                    effect: (() => {
        ...
        function createTaskDefinition() {
            return new ecs.TaskDefinition(...transform(args.transform?.taskDefinition, `${name}Task`, {
                ...
                containerDefinitions: $jsonStringify([
                    {
                        ...
                        pseudoTerminal: true,
                        portMappings: pub?.ports.apply((ports) => ports
                        ...
                        environment: all([args.environment ?? [], linkData]).apply(([env, linkData]) => [
        ...
        function createService() {
            return new ecs.Service(...transform(args.transform?.service, `${name}Service`, {
                name,
                cluster: cluster.arn,
                ...
                loadBalancers: targets &&
                    targets.apply((targets) => Object.values(targets).map((target) => ({
                        targetGroupArn: target.arn,
                        containerName: name,
        ...
        function createAutoScaling() {
            const target = new appautoscaling.Target(`${name}AutoScalingTarget`, {
        ...
        function registerReceiver() {
            self.registerOutputs({
                _dev: imageArgs.apply((imageArgs) => ({
                    ...
                    directory: output(args.dev?.directory).apply((dir) => dir ||
                        path.join(imageArgs.dockerfile
