import { interpolate, jsonStringify, output, } from "@pulumi/pulumi";
...
export class OpenSearch extends Component {
    constructor(name, args = {}, opts = {}) {
        ...
        const engineVersion = output(args.version).apply((v) => v ?? "OpenSearch_2.17");
        ...
        function reference() {
            ...
            const input = domain.tags.apply((tags) => {
                if (!tags?.["sst:ref:username"])
                    throw new VisibleError(`Failed to get username for OpenSearch ${name}.`);
                if (!tags?.["sst:ref:password"])
                    throw new VisibleError(`Failed to get password for OpenSearch ${name}.`);
                return {
                    username: tags["sst:ref:username"],
                    password: tags["sst:ref:password"],
                };
            });
            const secret = secretsmanager.getSecretVersionOutput({ secretId: input.password }, { parent: self });
            const password = $jsonParse(secret.secretString).apply((v) => v.password);
        ...
        function normalizeStorage() {
            return output(args.storage ?? "10 GB").apply((v) => {
                const size = toGBs(v);
                if (size < 10) {
                    throw new VisibleError(`Storage must be at least 10 GB for the ${name} OpenSearch domain.`);
                }
                return size;
            });
        }
        function registerDev() {
            ...
            if ($dev &&
                ...
                throw new VisibleError(`You must provide the password to connect to your locally running OpenSearch domain either by setting the "dev.password" or by setting the top-level "password" property.`);
            }
            const dev = {
                enabled: $dev,
...
Listening on "${dev.url}"...`,
        ...
        function createSecret() {
            const secret = new secretsmanager.Secret(`${name}Secret`, {
                recoveryWindowInDays: 0,
            }, { parent: self });
            new secretsmanager.SecretVersion(`${name}SecretVersion`, {
                secretId: secret.id,
        ...
        function createDomain() {
            return new opensearch.Domain(...transform(args.transform?.domain, `${name}Domain`, {
                engineVersion,
                clusterConfig: {
        ...
        function createPolicy() {
            return new opensearch.DomainPolicy(`${name}DomainPolicy`, {
